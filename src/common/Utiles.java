	...
	public static void DetectarEstado(Contrato c) {
		try {
			...
			if (hoy.isAfter(gcPost)) {
						...
						.forEach(
								item -> {
									int neoEstado = item.getArticulo()
											...
											&& item.getArticulo()
													.getEArticulo().getId() != EArticulo.BAJA ? EArticulo.SIN_PRECIO
											: EArticulo.VITRINA;
				...
				int conteoVitrina = (int) c.getDetalleContratos().stream()
						.filter(Constantes.predicadoConversorVitrina).count();
