...
public class Constantes {
	...
	public static final String[] ESTADOS_ALERTA = new String[] { "VENCIDO",
			"PRE", "POST", "VITRINA", "VITRINA (SP)" };
	public static final Integer[] ESTADOS_INACTIVIDAD = new Integer[] {
			EContrato.EN_PROCESO, EContrato.FUNDIDO, EContrato.CANCELADO,
			EContrato.VITRINA, 10, EContrato.REMATADO, EContrato.USO_OFICINA,
	...
	public static DefaultTableModel ClienteModel = new DefaultTableModel(null,
			new String[] { "ID", "DOC.IDENTIDAD", "NOMBRE", "A.PATERNO",
					...
					"DISTRITO", "CATEGORIA", "DETALLE", "STATUS" }) {
		public boolean isCellEditable(int rowIndex, int colIndex) {
			return false;
		}
	};
	...
	public static DefaultTableModel PrestamoModel = new DefaultTableModel(null,
			new String[] { "ID", "DESCRIPCIÓN", "INTERES (%)", "TIPO MORA",
					"MORA (%)", "TIPO" }) {
		public boolean isCellEditable(int rowIndex, int colIndex) {
			return false;
		}
	...
	public static DefaultTableModel UsuarioModel = new DefaultTableModel(null,
			new String[] { "ID", "LOGIN", "A.PATERNO", "A.MATERNO", "NOMBRE",
					"CLAVE", "TIPO", "INGRESO", "STATUS" }) {
		public boolean isCellEditable(int rowIndex, int colIndex) {
			return false;
		}
	};
	...
	public static ComboBoxModel TipoMoraModel = new DefaultComboBoxModel(
			new String[] { "%", "M" });

	public static ComboBoxModel TipoDocumentoModel = new DefaultComboBoxModel(
			new String[] { "DNI", "CEX", "PAS" });

	public static ComboBoxModel MonedaModel = new DefaultComboBoxModel(
			new String[] { "SOLES", "DÓLARES" });

	public static DefaultTableModel AbonoModel = new DefaultTableModel(null,
			new String[] { "FECHA PAGO", "CAPITAL ANTERIOR",
					"INTERES ANTERIOR", "IMPORTE ABONADO", "NUEVO CAPITAL",
					"NUEVO INTERES" }) {
		public boolean isCellEditable(int rowIndex, int colIndex) {
			return false;
		}
	...
	public static DefaultTableModel PagoModel = new DefaultTableModel(null,
			new String[] { "ID", "FECHA VCTO.", "FECHA PAGO", "CONCEPTO",
					"INTERES", "MORA", "TOTAL" }) {
		public boolean isCellEditable(int rowIndex, int colIndex) {
			return false;
		}
	...
	public static DateTimeFormatter formatoMes = DateTimeFormatter
			.ofPattern("MMMM");
	public static DateTimeFormatter formatoDiaHora = DateTimeFormatter
			.ofPattern("dd-MMM-yyyy HH:mm:ss");

	public static SimpleDateFormat formatoSQL_2 = new SimpleDateFormat(
			"yyyy-MM-dd");
	...
	public static Comparator<DetalleContrato> UltimoContratoComparator = new Comparator<DetalleContrato>() {
		...
		public int compare(DetalleContrato c1, DetalleContrato c2) {
			return LocalDate
					.parse(c1.getContrato().getFechaContrato())
					.compareTo(
							LocalDate
									.parse(c2.getContrato().getFechaContrato()));
		}
	};

	public static Predicate<Asistencia> predicadoAsistencia(String fechaCaja) {
		return t -> LocalDate.parse(t.getFecha()).compareTo(
				LocalDate.parse(fechaCaja)) == 0;
	};

	public static Predicate<Articulo> predicadoArticulo(String contrato) {
		return p -> p.getFlagContrato().concat("-")
				.concat(String.valueOf(p.getNumeroContrato()))
				.contains(contrato);
	};

	public static Predicate<Egreso> predicadoAnularEgreso(String contrato) {
		return e -> e.getDescripcion().startsWith(contrato)
				&& e.getTipo().equalsIgnoreCase("EMP")
	...
	public static Predicate<Component> predicadoCajaTexto = new Predicate<Component>() {
		...
		public boolean test(Component p) {
			return (p instanceof JLabel && ((JLabel) p).getBackground() == Color.WHITE) ? true
					: false;
		}
	};
